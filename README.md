# Region Hover for X / Twitter

A Chrome extension that shows the X-determined "based in" region for any account — just hover their profile picture.
...
4. Navigate to x.com — the extension captures auth tokens from normal page activity within a few seconds
...
### Auth token capture
X's web app sends a `Bearer` token and CSRF token with every API request. The extension's background service worker listens to outgoing requests via `chrome.webRequest.onBeforeSendHeaders` and captures these headers. They're stored in `chrome.storage.session` so they survive MV3 service worker sleep cycles.

### GraphQL query ID capture
X's internal GraphQL API uses opaque query IDs in URLs (e.g. `/graphql/zUnx-DLN9dkwOkNhTLySjg/AboutAccountQuery`). These IDs change with X deployments. The extension intercepts X's own requests and extracts them dynamically, falling back to known-good hardcoded values if they haven't been seen yet.

### On-demand fetching
When you hover a profile picture for 2 seconds, the extension:
1. Walks up the DOM from the hovered element to find a `<a href="/username">` wrapping a profile image
...
### 1. X overlays elements on top of profile images
The actual `<img>` element is never the top element when you hover a profile picture — X puts a transparent `<a>` or `<div>` on top for click handling. Checking `e.target.tagName === 'IMG'` always fails. Fix: on `mouseover`, walk up the ancestor chain looking for a `<a href="/username">` that *contains* an `img[src*="profile_images"]` child.

### 2. `UserByScreenName` does not contain the "based in" field
The `UserByScreenName` query — used for profile hover cards — only returns the user-set `location` free-text field. The X-determined `account_based_in` field lives in a separate query: **`AboutAccountQuery`**, which is only called when loading the `/about` tab of a profile. You have to make this second call explicitly.
...
Alternatives fire an API request for every visible tweet on page load, burning through rate limits quickly. Firing only on explicit hover with caching reduces typical session API calls from hundreds to single digits.

### 7. The `mouseout` timing matters
`mouseout` fires before `mouseover` fires on the next element. A hide delay shorter than the show delay causes the tooltip to never appear — the hide timer clears the pending show timer. The fix: match the hide delay to the show delay.

---

## Known limitations

- **No `account_based_in` for all accounts** — X only sets this field for some accounts. Many users will show only the user-set location.
...
All data is processed locally in your browser. No information is sent to any server controlled by this extension. See the full [privacy policy](privacy-policy.html).
